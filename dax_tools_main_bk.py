...
def add_measure():
    ...
    if edit_hwnd != 123:
        ...
        l = meta_dict['measures_list']
        if "[" in name and "]" in name:
            if name not in l:
                l.append(name)
            meta_dict['measures_list'] = l
...
def on_press(key):
    global num_flag, t_name, column_flag, edit_hwnd, thread_key_mouse_flag, edit_hwnd, input_win_title, q_list, q_dict
    ...
    if login_flag == "yes":
        ...
        if k == 'Key.space':  # 表名列名或者度量值中的名字有空格无法输入
            ...
            print(win_title2)
            # win_title_list = ["Power BI Desktop", "Tabular Editor", "记事本", "Microsoft Visual Studio", "PyCharm","DAX Tools",]
            if "DaxStudio" in win_title2:
        ...
        if k != "Key.ctrl_l" and str(key) != "'\\x16'":
            ...
            if k == "[":
                ...
                # for t in meta_dict['table_column_list']:
                #     if text_input.lower() in t.lower() and t not in q_list:
                #         q_list.append(t)
                #
                # q_list.sort()

                # q_list=xx
                # for index, iterm in enumerate(q_list):
                #     q_dict.update({index + 1: iterm})
            ...
            if '[' in key_list and column_flag == 'y' and k != 'None' and k != '/':
                ...
                # for c in meta_dict['column_dict'][t_name[0]]:
                #     if text_input.lower() in c.lower() and c not in q_list:
                #         q_list.append(c)
                # q_list.sort()
                #
                # for index, iterm in enumerate(q_list):
            ...
            if '[' in key_list and column_flag == 'n' and k != 'None' and k != '/':
                ...
                # for m in meta_dict['measures_list']:
                #     if text_input.lower() in m.lower() and m not in q_list:
                #         q_list.append(m)
                # q_list.sort()
                # for index, iterm in enumerate(q_list):
                #     q_dict.update({index + 1: iterm})
            ...
            if k == "`":
                ...
                global all_funs_list
                # for f in all_funs_list:
                #     if text_input.lower() in f.lower() and f not in q_list:
                #         q_list.append(f)
                # q_list.sort()
                # for index, iterm in enumerate(q_list):
                #     q_dict.update({index + 1: iterm})
...
def gui(databaseid, localhost):
    ...
    with window("info", show=False, width=360, height=290, x_pos=100, y_pos=300):
...
if __name__ == '__main__':
    ...
    try:
        ...
        # databaseid_value = ""
        # localhost_value = ""
        databaseid_value = "e9c385e9-e739-4ab4-a1f3-2e4c7691c5f2"
        localhost_value = "localhost:53114"
